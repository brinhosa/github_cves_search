...
def search_github_cve(cve_search):
    ...
    if cve_response.status_code == 200:
        ...
        if(args.total):
            total_items_count = len(data.get("items", []))
            print(cve_search+","+str(total_items_count))
            sys.exit()
        for item in data.get("items", []):
            html_url = item.get("html_url")
            if html_url:
                if(args.csv):
                    print(cve_search+","+html_url)
                else:
                    print(html_url)
...
def main():
    global kernel_version, cves, args
    ...
    parser.add_argument('-t', '--total', action='store_true', help='Return the total number of exploits')
    parser.add_argument('-csv', '--csv', action='store_true', help='Return in comma separated format')
    ...
    if(cve_search_str==""):
        ...
        #print("", cve_search_str,",",total_items_count)
